...
def eval_vision(worker, train_loader, valid_loader, epoch, iteration, tb, device):
    ...
    worker.grads_after_merge = [p.grad for p in params1 if p.grad is not None] 
...
def update_dsgd(worker_list, P, args, probe_valid_loader):
    ...
    for worker in worker_list:
        ...
        worker.update_grad()
        if worker.current_batch_index == worker.choose_batch and worker.rank == worker.choose_node and worker.train_to_end == False:
            worker.eval(probe_valid_loader, 0)
            print('valid acc', worker.valid_acc)
...
def evaluate_and_log(
    ...
    for worker in neighbor_worker:
        ...
        acc_average += valid_acc
        acc_train += train_acc
   
    
    estimation, dot1, dot2 = compute_estimation(choose_worker, neighbor_weight, neighbor_worker)
    
    valid_acc = acc_average / len(neighbor_worker)
    acc_train = acc_train / len(neighbor_worker)
    print(f' validation accuracy: {valid_acc:.4f} train accuracy: {acc_train:.4f}')
    return train_acc, train_loss, valid_acc, acc_train, valid_loss, estimation, dot1, dot2
...
def evaluate_last(worker_list, P, args, valid_loader):
    ...
    for worker in neighbor_worker:
        ...
        valid_acc.append(worker.valid_acc)
    ...
    valid_acc.append(choose_worker.valid_acc)
    ...
    return loss_list, loss_every_epoch, valid_acc
...
def compute_loss_every_node(loss):
    sum_ = 0
...
def read_json_file(file_path):
    ...
    with open(file_path, 'r', encoding='utf-8') as file:
        data = json.load(file)
        es_list = data['estimation_list']
        loss_evry_epoch = data['loss_every_epoch']
        choosenode_loss_list = loss_evry_epoch['choose_node']
        neighbornode_loss_list = loss_evry_epoch['neighbor_node']
        difference_list = dict()
        for i in range(len(es_list)):
            difference_list[f'epoch {i}'] = es_list[i] - ( choosenode_loss_list[0][0][i]-choosenode_loss_list[0][1][i] + sum(neighbor[0][i] for neighbor in neighbornode_loss_list) - sum(neighbor[1][i] for neighbor in neighbornode_loss_list))
        data['difference_list'] = difference_list
    with open(file_path, 'w', encoding='utf-8') as file:
        json.dump(data, file, indent=4, ensure_ascii=False)
        
def calculate_estimation_and_loss(json_file):
    with open(json_file, 'r', encoding='utf-8') as file:
            data = json.load(file)
            list1 = data['estimation_list']
            dict2 = data['difference_list']
            list2 = list()

            for i in range(0, len(dict2)):
                list2.append(dict2[f'epoch {i}'])
            # 计算前十、二十、三十、四十、五十个元素的差的和
            # loss_8 = list1[7] - list2[7]
            loss_2 = list1[1] - list2[1]
            # loss_5 = list1[4] - list2[4]
            # loss_15 = list1[14] - list2[14]
            # loss_20 = list1[19] - list2[19]
            # loss_30 = list1[29] - list2[29]
            # loss_10 = calculate_partial_difference_sum(list1, list2, 10)
            # loss_20 = calculate_partial_difference_sum(list1, list2, 20)
            # loss_30 = calculate_partial_difference_sum(list1, list2, 30)
            # loss_40 = calculate_partial_difference_sum(list1, list2, 40)
            # loss_50 = calculate_partial_difference_sum(list1, list2, 50)
            # loss_60 = calculate_partial_difference_sum(list1, list2, 60)
            ...
            # esti_20 = list1[19]
            # esti_30 = list1[29]
            # esti_10 = sum(list1[:10])
            # esti_20 = sum(list1[:20])
            # esti_30 = sum(list1[:30])
            # esti_40 = sum(list1[:40])
            # esti_50 = sum(list1[:50])
            # esti_60 = sum(list1[:60])
            
            # data['8 epoch estimation and loss'] = [esti_8, loss_8]
            data['2 epoch estimation and loss'] = [esti_2, loss_2]
            # data['5 epoch estimation and loss'] = [esti_5, loss_5]
            # data['15 epoch estimation and loss'] = [esti_15, loss_15]
            # data['20 epoch estimation and loss'] = [esti_20, loss_20]
            # data['30 epoch estimation and loss'] = [esti_30, loss_30]
            # data['10 epoch estimation and loss'] = [esti_10, loss_10]
            # data['20 epoch estimation and loss'] = [esti_20, loss_20]
            # data['30 epoch estimation and loss'] = [esti_30, loss_30]
            # data['40 epoch estimation and loss'] = [esti_40, loss_40]
            # data['50 epoch estimation and loss'] = [esti_50, loss_50]
            # data['60 epoch estimation and loss'] = [esti_60, loss_60]
