...
from .mlp import MLP, Simple, ImagenetMLP
from .cnn import FlexibleCNN
from .cnn_imagenet import SimpleCNN
...
def load_model(name, outputsize, pretrained=None, args=None, nb_class=10):
    ...
    if name.lower() in 'alexnet_micro':
        ...
        if args.dataset_name == 'mnist':
            ...
            model = ImagenetMLP(input_channels=3, image_size=args.image_size, num_classes=nb_class)
        else:
    ...
    elif name.lower() in 'cnn_imagenet':
        model = SimpleCNN(num_classes=nb_class)
